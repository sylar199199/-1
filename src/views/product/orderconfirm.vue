    ...
    <div class="orderconfirm" v-show="showData">
            ...
            <div class="orderBottom">
                ...
                <a class="mint-cell mint-field" v-if="content.purchaseNotesFlag == 1">
                    <div class="mint-cell-wrapper">
                        <div class="mint-cell-title">
                        <span class="mint-cell-text">
                            <img class="confirmIcon" v-if="noSelect" @click="noSelect = false" src="../../assets/image/weixuan.png">
                            <img class="confirmIcon" v-if="!noSelect" @click="noSelect = true" src="../../assets/image/xuanzhong.png">
                        </span>
                        </div>
                        <div class="mint-cell-value">
                ...
                <div class="orderPrice">
                    ...
                    <div class="smallButton font28" style="width: 100%;" v-if="rightsFlag == '01'" @click="confirmOrder()">领取</div>
                </div>
            </div>
        </div>
        ...
        <mt-popup  v-model="popupadressWrite" position="bottom" class="popGuige">
            ...
            <div class="popContent">
                <div class="popInput">
                    ...
                    <input type="text"  id='inputone' v-model="appnt.name" @change="watchChange('name')" class="inputText" placeholder="收货人姓名(请使用真实姓名)"/>
                </div>
                <div class="popInput">
                    <span class="contentTitle">手机号</span>
                    <input type="tel" id='inputtwo' v-model="appnt.mobile" @change="watchChange('mobile')" class="inputText" placeholder="手机号码"/>
                </div>
                <div class="popInput">
                    <span class="contentTitle">地区</span>
                    <input type="text"  class="inputText" v-model="appnt.areaText" @click="choiceArea" readonly placeholder="所在地区"/>
                </div>
                <div class="popInput">
                    <span class="contentTitle">收货地址</span>
                    <input type="text" id='inputthree' v-model="appnt.address" @change="watchChange('addresscount')" class="inputText" placeholder="街道、小区门牌等详细地址"/>
                </div>
            </div>
        ...
        <mt-popup  v-model="popupinfoWrite" position="bottom" class="popGuige popinfo">
            ...
            <div class="popContent">
                <div class="popInput">
                    <span class="contentTitle">姓名</span>
                    <input type="text"  id='inputfour' v-model="info.name" @change="watchChange('name')" class="inputText" placeholder="联系人姓名 (请使用真实姓名)"/>
                </div>
                <div class="popInput">
                    <span class="contentTitle">手机号</span>
                    <input type="tel" id='inputfive' v-model="info.mobile" @change="watchChange('mobile')" class="inputText" placeholder="手机号码"/>
                </div>
                <div class="popInput">
                    <span class="contentTitle">身份证</span>
                    <input type="tel" id='input0' v-model="info.idNum" @change="watchChange('idNum')" class="inputText" placeholder="身份证号"/>
                </div>
                <div class="popInput">
                    <span class="contentTitle">性别</span>
                    <span :class="selectSexone?'sexSpan font24 color2c sexSpanSelect':'sexSpan font24 color1e sexSpannoSelect'" @click="selectSexone=true;selectSextwo=false" >男</span>
                    <span :class="selectSextwo?'sexSpan font24 color2c sexSpanSelect':'sexSpan font24 color1e sexSpannoSelect'" @click="selectSexone=false;selectSextwo=true" >女</span>
                </div>
                <div class="popInput" style="display: flex">
                    ...
                    <div class="mint-cell-value">
                        <div class="mint-cell-value is-link inputTextone font24 color1e" @click="openPicker('insDatePicker')">
                            <input placeholder="请选择日期" type="datetime" class="list-field-core" readonly="readonly" v-model="info.birthday">
                ...
                <div class="popInput healthbox">
                    ...
                    <textarea id='inputsix' v-model="info.healthDescription" @change="watchChange('mobile')" class="textareaText" placeholder="健康描述"></textarea>
                </div>
            </div>
            <div class="submitOrderBox">
        ...
        <mt-popup  v-model="popupcityWrite" position="bottom" class="popGuige popcity">
            ...
            <div class="popinfoTitle font28 color1e">该服务目前支持以下7个城市，请选择您最方便到现场
                的城市：</div>
            <div class="popContent">
                <span @click="selectCity(item)" :class="item.isSelect?'hasSelcetBox font24 color2c':' selectCity font24 color1e'" v-for="item in cityData">{{item.name}}</span>
            </div>
            <div class="submitOrderBox">
                <div class="submitOrder font28" @click="submitone()">完成</div>
            </div>
        ...
        <mt-datetime-picker ref="insDatePicker" v-model="dateValue" type="date" :startDate="startDate" :endDate="endDate"
                            year-format="{value} 年" month-format="{value} 月" date-format="{value} 日" @confirm="insBirChange">
        </mt-datetime-picker>
