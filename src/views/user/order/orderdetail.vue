...
<template>
    <div class='orderDetail'>
        ...
        <div class="statusBac" :style="{backgroundImage: 'url(' + imageUrl + ')',backgroundRepeat: 'no-repeat'}">
            <div v-if="afs">
                <div :class="afs.status == 7 || afs.status == 10?'orderDeatileTitle color1d font36':'orderDeatileTitlep color1d font36'">{{statusTitle}}</div>
            </div>
            <div v-if="!afs">
                <div :class="orderDetail.status == 1?'orderDeatileTitle color1d font36':'orderDeatileTitlep color1d font36'">{{statusTitle}}</div>
                <div class="orderDeatileTime color97 font24" v-if="orderDetail.status == 6 ">{{orderDetail.message}}</div>
                <div class="orderDeatileTime color97 font24" v-if="orderDetail.status == 1 && (minute && second && hour)">距离订单关闭<span v-if="minute!='00'">{{hour}}时{{minute}}分</span>{{second}}秒</div>
            </div>
            <div v-if="afs">
                ...
                <div class="orderDeatileTime color97 font24" v-if=" afs.status == 2 || afs.status == 5">拒绝原因：{{afs.message}}</div>
                <div class="orderDeatileTime color97 font24"  v-if="afs.status == 2 || afs.status == 5" style="padding-bottom:10px;">有疑问请拨打官方客服电话 <a class="color97 font24" href="tel:400-811-8899">400-811-8899</a></div>
            </div>
        </div>
        ...
        <div class="showAdress" v-if="orderDetail.express">
            ...
            <div class="LogisticsNumber"  v-if="orderDetail.express.expressNo">
                ...
                <span class="font24 color97 fr" :data-clipboard-text="orderDetail.express.expressNo" id="tags" @click="copy()">复制单号</span>
            </div>
        </div>
        <div class="showAdress productInfo"  v-if="orderDetail.status == 4 || orderDetail.status == 5">
            <div class="infoBox">
                ...
                <div class="fr" @click="goProductDes()" v-if="usageInstructionsFlag == 1">
            ...
            <div class="cardInfo">
                ...
                <div class="cardPassword flexBetween" v-if="card.password">
                    <div class="passwordBox">
                        ...
                        <span class="viewsAll color97 font24" @click="showAll = true">查看全部</span>
                    </div>
                    <div class="smallButton" v-if="orderDetail.planActivateNowFlag == '01' && orderDetail.status == 4" @click="goUse()">去使用</div>
                </div>
            </div>
        </div>
        ...
        <div class="showAdress" v-if="orderDetail.goodsType == 2 && consignee">
            <div class="infoBox">
        ...
        <div class="treasureBox">
            <div class="borderBox">
                <div class="popImgBox">
                    <div class="popImg" :style="{backgroundImage: 'url(' + orderDetail.goodsImgUrl + ')',backgroundRepeat: 'no-repeat'}"></div>
                    <div class="popGuigeTitle">
                        <p class="popprice">
                            <span class="productName color1d">{{orderDetail.goodsName}}</span>
                            <span class="price color1e" v-if="orderDetail.orderType == '01'">¥{{orderDetail.goodsPrice}}</span>
                        </p>
                        <p class="kucun">
                ...
                <div class="clear treasureBottom" v-if="orderDetail.orderType == '01'">
                    <span class="orderMoney colord8 font32 fr" @click.stop='goPay(item.id)'>¥{{orderDetail.goodsPrice}}</span>
                ...
                <div v-if="!afs && orderDetail.orderType == '01'">
                    ...
                    <div class="clear treasureBottom" v-if="orderDetail.status == 2 || orderDetail.status == 3 || orderDetail.status == 4">
                        <span class="smallgreenButton fr" @click.stop='goApplayreturn()'>申请退货</span>
                        <span class="smallButton fr" style="margin-right: 10px;"  v-if="!orderDetail.invoice && invoiceType == 1"  @click.stop='openInvoice()'>申请发票
                         </span>
                        <span class="smallButton fr" style="margin-right: 10px;" v-if="orderDetail.invoice && invoiceType == 1"  @click.stop='goInvoice()'>已申请发票
                        </span>
                    </div>
                    <div class="clear treasureBottom" v-if="orderDetail.status == 5">
                        <span class="smallgreenButton fr" @click.stop='confirmToast()'>申请退货</span>
                        <span class="smallButton fr" style="margin-right: 10px;"  v-if="!orderDetail.invoice && invoiceType == 1"  @click.stop='openInvoice()'>申请发票
                         </span>
                        <span class="smallButton fr" style="margin-right: 10px;" v-if="orderDetail.invoice && invoiceType == 1"  @click.stop='goInvoice()'>已申请发票
                        </span>
                ...
                <div v-if="afs  && orderDetail.orderType == '01'">
                    <div class="clear treasureBottom" v-if="afs.status == 3">
                        <span class="smallgreyButton fr" @click.stop='gostatusDetail()'>同意退货</span>

                    </div>
                    <div class="clear treasureBottom" v-if="afs.status == 2">
                        <span class="smallgreyButton fr" @click.stop='gostatusDetail()'>拒绝退货</span>
                        <span class="smallButton fr" style="margin-right: 10px;"  v-if="!orderDetail.invoice && invoiceType == 1"  @click.stop='openInvoice()'>申请发票
                         </span>
                        <span class="smallButton fr" style="margin-right: 10px;" v-if="orderDetail.invoice && invoiceType == 1"  @click.stop='goInvoice()'>已申请发票
                        </span>
                    </div>
                    <div class="clear treasureBottom" v-if="afs.status == 1">
                        <span class="smallgreyButton fr" @click.stop='gostatusDetail()'>退货中</span>
                    </div>
                    <div class="clear treasureBottom" v-if="afs.status == 4">
                        <span class="smallgreyButton fr" @click.stop='gostatusDetail()'>退款中</span>
                        <!--<span class="smallButton fr" style="margin-right: 10px;"  @click.stop='openInvoice()'>申请发票-->
                        <!--</span>-->
                    </div>
                    <div class="clear treasureBottom" v-if="afs.status == 5">
                        <span class="smallgreyButton fr" @click.stop='gostatusDetail()'>拒绝退款</span>
                        <span class="smallButton fr" style="margin-right: 10px;" v-if="!orderDetail.invoice && invoiceType == 1"  @click.stop='openInvoice()'>申请发票
                     </span>
                        <span class="smallButton fr" style="margin-right: 10px;" v-if="orderDetail.invoice && invoiceType == 1"  @click.stop='goInvoice()'>已申请发票
                        </span>
                    ...
                    <div class="clear treasureBottom" v-if="afs.status == 6">
                        <span class="smallgreyButton fr" @click.stop='gostatusDetail()'>退款详情</span>
                    </div>
                </div>
        ...
        <div class="showAdress orderInfo" v-if="orderDetail.status != 1">
            ...
            <div class="cardInfo">
                ...
                <div class="addressBoxone font24 color1e" v-if="(orderDetail.payTime)">支付时间：{{timetrans(orderDetail.payTime)}}</div>
            </div>
        </div>
        <mt-popup  v-model="showInvoice" position="bottom" class="popGuige popInvioce">
            <div class="poptitlebox">
                <span class="popTitle">申请发票</span>
                <div class="popup-close" @click="showInvoice=false">
                </div>
            </div>
            <div class="popContent">
                ...
                <div class="popInput healthbox">
                    ...
                    <span :class="item.isSelect?'sexSpan font24 color2c sexSpanSelect':'sexSpan font24 color1e sexSpannoSelect'"  v-for="item in headTypeOption" @click="selectType(item)">
                        {{item.value}}
                    </span>
                </div>
                <div class="popInput healthbox"  v-if="invoice.headType == 2">
                    <span class="contentTitle healthTitle">纳税人类别</span>
                    <span :class="item.isSelect?'sexSpan font24 color2c sexSpanSelect':'sexSpan font24 color1e sexSpannoSelect'"  v-for="item in payerTypeOption" @click="selectpaerType(item)">
                        {{item.value}}
                    </span>
                </div>
                <div class="popInput" v-if="invoice.headType == 2">
                    <span class="contentTitle">发票抬头</span>
                    <input type="text"  id='inputfour' v-model="invoice.invoicePayable" @change="watchChange('invoicePayable','')" class="inputText" placeholder="请填写发票抬头"/>
                </div>
                <div class="popInput" v-if="invoice.headType == 1">
                    <span class="contentTitle">发票抬头</span>
                    <input type="test" v-if="!realName" v-model="invoice.realNameone" @change="watchChange('realNameone','')" class="inputText" placeholder="请填写发票抬头"/>
                    <span v-if="realName">{{realName}}</span>
                </div>
                <div class="popInput" v-if="invoice.headType == 2">
                    <span class="contentTitle">发票税号</span>
                    <input type="text" id='inputfive' v-model="invoice.invoiceNumber" @change="watchChange('invoiceNumber','')" class="inputText" placeholder="请填写发票税号"/>
                </div>
                <div class="popInput healthbox">
                ...
                <div class="popInput">
                    ...
                    <input type="text" id='input0' v-model="invoice.phone" @change="watchChange('phone','')" class="inputText" placeholder="请填写手机号"/>
                </div>
                <!--<div class="popInput">-->
            ...
            <div class="submitOrderBox">
                <div class="submitOrder font28" @click="submitInvoice()">提交</div>
            </div>
        </mt-popup>
        <mt-popup  v-model="popupresonWrite" position="bottom" class="popGuige popcity">
            <div class="poptitlebox">
                <span class="popTitle">申请退货</span>
                <div class="popup-close" @click="popupresonWrite=false">
                </div>
            </div>
            <div class="popinfoTitle font28 color1e">
                退货原因：
            </div>
            <div class="returnReason">
                <div class="selectBox" v-for="item in returnReasonData" @click="selectReason(item)">
                    <img v-if="!item.isSelect" class="selectIcon" src="../../../assets/image/weixuan.png">
                    <img v-if="item.isSelect" class="selectIcon" src="../../../assets/image/xuanzhong.png">
    ...
    export default {
        ...
        data() {
            return {
                ...
                returnReasonData: [{value:'产品质量问题',id:1,isSelect:false},{value:'不再需要了',id:2,isSelect:false},{value:'其他',id:3,isSelect:false}],
        ...
        methods: {
            ...
            goInvoice(){
                this.$router.push({name:'invioceDetail',params: {
                ...
                Service.order().getorderDetail({
                    ...
                    if (response.errorCode == 0) {
                        ...
                        if(timeone>0 && this.orderDetail.status == 1){
                            ...
                            // this.hour = parseInt(time/3600)%24>9?parseInt(time/3600)%24:'0'+parseInt(time/3600)%24;
                            // this.minute = parseInt(time/60)>9?parseInt(time/60):'0'+parseInt(time/60);
                            // this.second = parseInt(time%60)>9?parseInt(time%60):'0'+parseInt(time%60);
                            // var that = this;
                            // var t = setInterval(function() {
                            //     that.second--;
                            //     if(that.hour == '00' && that.second == '00' && that.minute == '00') {
                            ...
                            //         that.$forceUpdate()
                            //     }; //当分钟和秒钟都为00时，重新给值
                            //     if(that.second == '00' && that.minute != '00') {
                            //         that.second = 59;
                            //         that.minute--;
                            //         if(that.minute < 10){
